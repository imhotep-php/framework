...
class ContainerCallTest extends TestCase
{
    ...
    {
        $container = new Container;
        $result = $container->call('Imhotep\Tests\ContainerNew\ContainerStaticMethodStub::inject');
        $this->assertInstanceOf(ContainerCallConcreteStub::class, $result[0]);
        $this->assertSame('taylor', $result[1]);
    }
    ...
    {
        $container = new Container;
        $result = $container->call('Imhotep\Tests\ContainerNew\containerTestInject');
        $this->assertInstanceOf(ContainerCallConcreteStub::class, $result[0]);
        $this->assertSame('taylor', $result[1]);
    }
    ...
    public function testCallWithoutRequiredParamsThrowsException()
    {
        ...
        $this->expectExceptionMessage('Unable to resolve dependency [Parameter #0 [ <required> $foo ]] in class Imhotep\Tests\ContainerNew\ContainerTestCallStub');
    ...
    {
        $this->expectException(BindingResolutionException::class);
        $this->expectExceptionMessage('Unable to resolve dependency [Parameter #0 [ <required> $foo ]] in class Imhotep\Tests\ContainerNew\ContainerCallTest');
