...
use Imhotep\Database\Sqlite\SchemaGrammar;
...
class SqliteSchemaGrammarTest extends TestCase
{
    ...
    protected function getStatements(Closure $callback)
    {
        ...
        return $table->toSql(new SchemaGrammar());
    }
    ...
    public function test_add_base_data_types_columns()
    {
        ...
        $expected = [
            ...
            'ALTER TABLE "test" ADD COLUMN "deleted_at" TIMESTAMP',
        ];
    ...
    public function test_foreign_key()
    {
        ...
        $this->assertCount(1, $statements);
        $this->assertSame('CREATE TABLE "test" ("foo" TEXT NOT NULL, "bar_id" INTEGER NOT NULL DEFAULT "0", PRIMARY KEY ("foo"), FOREIGN KEY ("bar_id") REFERENCES "bars"("id") ON DELETE SET DEFAULT ON UPDATE CASCADE)', $statements[0]);
    }
    ...
    public function test_foreignId()
    {
        $statements = $this->getStatements(function (Table $table) {
            ...
            $table->foreignId('foo_bar_id')->constrained();
            $table->foreignId('item_id')->references('id')->on('items');
            $table->foreignId('item_foo_id')->references('id')->on('items');
        });
