...
class SchemaGrammar extends BaseSchemaGrammar
{
     ...
     * @var string[]
     */
    protected array $serials = ['id', 'integer', 'smallInteger', 'bigInteger', 'bigSerial', 'serial', 'smallSerial'];
    ...
    */
    ...
    public function compileHasDatabase(string $name): string
    {
        return sprintf('SELECT 1 FROM pg_database WHERE datname = %s', $this->quoteString($name));
    }
    ...
    public function compileGetTables(string $searchPath = 'public'): string
    {
    ...
    protected function compileAutoIncrementStartingValues(Table $table, &$sql): void
    {
    ...
    public function compileChange(Table $table, Fluent $command): array
    {
        $sql = [];
    ...
    public function compileFulltext(Table $table, Fluent $command): string
    {
    ...
    public function compileSpatial(Table $table, Fluent $command): string
    {
    ...
    public function compileDropFullText(Table $table, Fluent $command): string
    {
        return $this->compileDropIndex($table, $command);
    }

    public function compileDropSpatialIndex(Table $table, Fluent $command): string
    {
        return $this->compileDropIndex($table, $command);
    }

    public function compileDropForeign(Table $table, Fluent $command): string
    {
    ...
    {
        return $this->typeBigSerial($column->autoIncrement());
    }
